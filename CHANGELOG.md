...
- 新增归档自动下载功能：支持在归档页面自动点击下载按钮（原始档案、重采样档案、H@H 下载）
- 新增 H@H 下载画质选择：支持原图、2560x、1920x、1280x、800x 分辨率，不可用时自动降级
- 新增 H@H 下载完成后自动关闭归档标签页
- 新增 hath.network 归档页面自动点击下载链接
- 新增归档页面繁体中文翻译回退支持（下載器、原圖、免費 等）
- 修复 Manifest V3 下 Service Worker 重启时右键菜单重复注册的问题
- 新增 `unlimitedStorage` 权限，避免存储配额溢出
- 修复 CI 中 CRX_KEY 为空时构建失败的问题
- 修复 webpack 配置中未指定 `env.type` 时构建失败的问题
